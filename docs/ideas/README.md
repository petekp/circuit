...
| [`e2-equipment-scope-spec.md`](e2-equipment-scope-spec.md) | `pre-implementation` | B3 spec: equipment scope (primitive 3b) as a declared, manifest-first field with write-tier enforcement, turning decomposition grain into a per-step dial. Design only. |
| [`equipment-scope-build-brief.md`](equipment-scope-build-brief.md) | `implemented` | Build brief for the equipment-scope mechanism (declared tools sub-axis + write-tier enforcement). Both increments implemented on feat/equipment-scope (held PR): declared+trusted provide, plus enforced write-tier restriction via claude-code --tools with honest downgrade on incapable connectors. |
...
| [`paper-to-site-flow-brief.md`](paper-to-site-flow-brief.md) | `partially-implemented` | Build brief for the explainer flow, the operator's research-paper to interactive-site pipeline codified as the first hand-authored, non-build-shaped flow. A runnable v1 is authored (held PR): the end-to-end spine of intake, lossless digest, persona-lensed ideation, a six-criteria tournament, an adversarial fidelity hardening pass, an operator pick, a house-style spec, a child build sub-run, verification, an operator fidelity sign-off, and an honest close. Internals are deliberately thin pending a second-paper refinement pass. |
...
| [`decision-layer-exploration.md`](decision-layer-exploration.md) | `current-strategy-context` | Framing lens (design sketch, not a build plan) for making Circuit's per-step decisions (how to chop, what tools, which context) first-class and trialable. Applies the blocks-and-recursion move to the decisions about work, with explicit restraint: earn the unified resolver abstraction from concrete instances, do not build it first. |
...
| [`exploration-substrate-two-track-plan.md`](exploration-substrate-two-track-plan.md) | `current-proposal` | Plan separating the first-class-composition M-spine from an exploration substrate for measured experiments. E1 is its first milestone. Plan context, not shipped behavior. |
| [`mini-harness-debrief-vs-circuit.md`](mini-harness-debrief-vs-circuit.md) | `current-comparison` | Compares an external exploratory mini-harness against Circuit's first-class composition direction. Comparison context that motivates the exploration substrate. |
...
| [`durability-tier2-cursor-spec.md`](durability-tier2-cursor-spec.md) | `implementation-spec` | Decision-ready spec for Tier-2 forward recovery, companion to the fallible-executor audit. One slice is built (`RecoveryCorridor.seedFromTrace`, structural-only, wired inert behind resume); the rest is the fork (resumable cursor vs projection/checkpoint-only + Tier-3 restart-cheapness), three options (A general cursor, B bounded linear-only cursor, C foundation + Tier-3), and a recommendation to bank Rank-1 now and surface Option B. Two load-bearing unknowns: per-step idempotency and the app→runtime staleness boundary. |
...
| [`durability-tier3-restart-linkage-spec.md`](durability-tier3-restart-linkage-spec.md) | `pre-implementation` | Surface-only, decision-ready spec resolving the open question the linkage spec flagged: with no cursor (Option C) and a new parent id per restart, what consumes skip-finished? Recommends an explicit prior-run pointer (`circuit run --reuse-children-from <dead-run-folder>`) that reuses prior children by their stable (step_id, attempt, branch_id) address, never resumes the dead folder, and sidesteps the not-idempotent-relay blocker by reusing results not re-running steps. Reanimation rejected as the deferred cursor; content-addressed store as disproportionate. Inherits the refuse-on-shared-checkout rule and a staleness gate. Not gated on the cursor; not built. |
