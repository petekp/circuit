...
| [`decision-layer-exploration.md`](decision-layer-exploration.md) | `current-idea` | Exploration of a uniform per-axis chooser (a resolver) mapping task context plus prior choices to one choice for one axis, materialized onto an assembly spec. Section 7 records the as-if-unified shape. Two instances prototyped offline; nothing wired into production authoring yet. |
...
| [`next-phase-build-brief.md`](next-phase-build-brief.md) | `partially-implemented` | Build brief for an offline flow lab plus the first two resolvers (structure chop/hold and equipment skill-injection). The lab and both thin resolvers are built under experiments/ with zero engine change; the deep unit-unification refactor is left as the operator's ratification item. |
| [`next-phase-run-report.md`](next-phase-run-report.md) | `partially-implemented` | Run report for the offline build: the flow lab and quality ratchet, the two resolvers with their shared shape and four divergences, the typed-seam authoring constraint, and a proposed production plan. Built offline with no model spend; nothing wired into production authoring yet. |
