...
> operator-noted craft gaps (responsive layout + animations). The headline
> **recovery-binding hard-abort is now fixed on `main`** (PR #105 threads
> `recoveryRouteBindings` on resume, so a failed child degrades onto its recovery
> route instead of hard-aborting the resumed parent); the rest remain open. The
