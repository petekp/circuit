...
### Update — 2026-06-17 (after the overnight reliability batch)

The original list above is the point-in-time record as of HEAD `af7345d2`. Four of
those six items moved when work landed on `main` in parallel (HEAD is now
`f4d260b1`). This addendum reconciles them; the north-star
([`north-star-status.md`](north-star-status.md)) carries the live detail.

- **Step 2 (live equipment-injection reshape) — now SHIPPED** (PR #103, merge
  `4fa7dae4`). Additive equipment injection: a relay bubbles up a confirmed
  `equipment_discovery`, the runner re-equips the remaining relay steps and
  re-compiles through the existing chain, no splice seam. The splice seam and
  structural re-decomposition (Step 3) remain deferred behind ratification, and the
  report recommends landing the operator surface (F2) first. Report:
  [`step2-live-equipment-reshape-report.md`](step2-live-equipment-reshape-report.md).
- **Durability re-entry for skip-finished children — now BUILT** (PR #106, merge
  `5fef310f`). `circuit run --reuse-children-from` reuses a dead run's finished
  sub-run children by their stable `(step_id, branch_id)` address behind a four-gate
  safety floor that fails safe to a fresh run. **Documented limitation:** the child
  flow version and base commit are not checked — it assumes the same flow at the
  same goal. The bulk-resume inbox driver is still surfaced.
- **Paper-to-site recovery-binding hard-abort — now FIXED** (PR #105, merge
  `748a14ee`). The resume path now threads `recoveryRouteBindings`, so a failed
  sub-run child degrades onto its recovery route instead of hard-aborting the
  resumed parent. The other paper-to-site findings (post-editorial checkpoint,
  greenfield-scaffold gap, craft gaps) remain open.
- **On-demand context pull — now SPIKED offline** (PR #107). A pure-function
  demonstrator in `experiments/flow-lab/` reaches fat-push completeness at about a
  tenth of the carried bytes, with the conservative defaults (no `*` query, bounded
  budget, legible trace) measured. No live/`src/` channel yet; still sequenced after
  the live recompile work.

**Still genuinely unresolved (unchanged):** the resolver abstraction is deliberately
not extracted (awaiting a third instance), and the grain verdict is still NULL (needs
tasks that actually induce false fixing).
