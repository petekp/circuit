...
> **Update — 2026-06-17: fixed on `main` (PR #105).** The engine-level root cause
> was that the resume path (`resumeCompiledFlowResult`) passed `workContractRef` but
> not `recoveryRouteBindings`, so on resume the graph-runner defaulted the binding
> list to `[]` and a recovery verdict found no match. The fix threads the same
> bindings the top-level path projects, so a failed sub-run child now degrades onto
> its recovery route instead of hard-aborting the resumed parent. See
> [`overnight-reliability-run-report.md`](overnight-reliability-run-report.md)
> (CHUNK 1). The separate P0 below — a checkpoint after editorial — remains open.
