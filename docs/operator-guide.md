...
| Review | Auditing one working tree, staged or unstaged set, commit, or Git range, or the code at a path as it stands today. Name the target in the goal; a goal that names none reviews the current working tree and says so in the report. Narrow it with a path if you want: "only in src/", "except tests/", or a bare "review src/runtime" all review just those paths, and the report names the scope. Point it at a path with nothing changed and it reads the current contents instead of a diff, and says which one you got: "review src/runtime for latent issues" and "review src/runtime as it stands" both ask for that directly. A snapshot reads tracked files only, and says how many it read out of how many the path matched. "Except untracked files" narrows to the tracked changes. A narrowing Circuit cannot express, such as "except the deleted files", reviews the whole target and says which narrowing it could not apply. For a plan or report, include its actual text in the request; a file path alone is not evidence. Circuit does not fetch pull requests: check the branch out first, then review the working tree or a range. | Audit-only. |
...
| --- | --- |
| `./bin/circuit preview [flow] [--power <tier>] [--matrix]` | Show which connector, model, and effort each step would get, without spawning anything. With no flow named it surveys every public flow at the chosen dial. `--matrix` prints every power setting at once, for one flow or for all of them. |
| `./bin/circuit doctor [--json]` | Confirm your runs would actually start, before one spends anything. Exits 0 once every chosen connector is ready and every public flow could dispatch under your config. Each problem comes with the fix. When doctor names a config key as the cause, it has checked that removing that key clears that blocker, so the `circuit config unset` line it prints is one you can paste. When no single unset would clear it, doctor reports the blocker without naming a key rather than guessing. The CHOSEN BY column names the config decision behind each chosen connector; connectors no flow chooses show `-`, are optional, and never fail the check. |
