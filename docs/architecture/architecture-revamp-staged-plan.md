...
- Stage 3 file-surface declarations — complete on `pkp/architecture-revamp-stage-3-declaration-seams-v2`; `fix.change-set@v1` and `build.plan@v1` file surfaces are now declared on their flow-owned report declarations and projected into Skill Hooks, with `npm run check-flow-drift` passing after regenerating host runtime bundles.
- Stage 3 Prototype connector-planning edge removal — complete on `pkp/architecture-revamp-stage-3-prototype-planning`; Prototype variant connector planning now goes through the neutral `src/selection/connector-planning.ts` contract with a runtime-facing connector adapter, the flow-package `src/connectors` allow-list is empty, and the top-level cycle ratchet dropped the two connector-rooted cycles. Focused Stage 3 connector tests, `npm run check-flow-drift`, and `npm run verify` passed.
- Stage 3 pure flow-kind policy checker — complete on `pkp/architecture-revamp-stage-3-flow-kind-policy-seam`; `checkCompiledFlowKindCanonicalPolicyWithTable` now accepts caller-supplied canonical policy tables while the existing wrapper still uses the catalog-derived table. Focused flow-kind tests, `npm run check-flow-drift`, and `npm run verify` passed after regenerating host runtime bundles.
- Stage 4 `src/policy -> src/flows` edge removal — complete on `pkp/architecture-revamp-stage-4-policy-edge`; flow-kind policy checks are table-parameterized, the catalog-bound adapter lives on the flow-owned canonical-stage policy surface, the `src/policy -> src/flows` ratchet allow-list is empty, and the top-level cycle ratchet dropped `flows -> policy -> flows` plus `flows -> shared -> policy -> flows`. Focused Stage 4 policy tests, `npm run check-flow-drift`, and `npm run verify` passed.
- Stage 4 selection shared-edge removal — complete on `pkp/architecture-revamp-stage-4-selection-edge`; relay selection and selection resolution now live in `src/selection`, runtime passes the flow-owned depth-binding flag as data, `src/selection` keeps its schema-only ratchet, and the `src/shared -> src/flows` allow-list is down to `operator-summary-writer.ts` and `relay-support.ts`. Focused architecture, selection, relay-guidance, and CLI depth-selection tests, `npm run check-flow-drift`, and `npm run verify` passed.
- Stage 4 operator-summary shared-edge removal — complete on `pkp/architecture-revamp-stage-4-operator-summary-edge`; the operator summary writer now lives in `src/app/operator-summary/`, accepts the existing intentional `src/app -> src/flows` dependency, leaves shared projection helpers flow-free, and the `src/shared -> src/flows` allow-list is down to `relay-support.ts`. Focused architecture, operator-summary, Skill Hooks, CLI output, and checkpoint-boundary tests, `npm run check-flow-drift`, and `npm run verify` passed.
- Stage 4 relay-support shared-edge removal — complete on `pkp/architecture-revamp-stage-4-relay-support-edge`; relay prompt support now lives in `src/runtime/run/`, the `src/shared -> src/flows` allow-list is empty, and the top-level cycle ratchet dropped `flows -> shared -> flows`. The branch also fixed the emitter check-mode stale-surface snapshot so the existing drift characterization stays reliable. Focused architecture/runtime relay-support tests, `tests/unit/emit-flows-drift.test.ts`, `npm run check-flow-drift`, and `npm run verify` passed.
- Stage 4 memory/history core extraction — complete on `pkp/architecture-revamp-stage-4-memory-history-core`; run-corpus discovery now lives in `src/history/run-corpus.ts`, app history and memory distillation both use that lower core, `src/memory -> src/app` is empty, and the top-level cycle ratchet is empty. Focused architecture/history/distill tests, `npm run check-flow-drift`, and `npm run verify` passed.
- Stage 5 recovery route policy centralization — complete on `pkp/architecture-revamp-stage-5-recovery-policy`; recovery route cause/ref contract rules now live in `src/schemas/recovery-route-kind.ts`, projection selection/authority/budget policy lives in `src/policy/recovery-route-policy.ts`, same-step retry remains target-sensitive, and dormant recovery kinds are explicit. Focused recovery-route, WorkContract projection, runtime-baseline, and architecture tests, `npm run check-flow-drift`, and `npm run verify` passed.
- Stage 5 StepKind alignment ratchet — complete on `pkp/architecture-revamp-stage-5-stepkind-ratchet`; `tests/contracts/step-kind-alignment.test.ts` now compares the runtime `StepKind` union, schema Step variants, `ExecutableStep` union, executable conversion, executor registry, and WorkContract projection keys without adding a runtime metadata table. Focused StepKind/schema/runtime-index tests, `npm run check`, and `npm run verify` passed.
- Stage 5 transition classifier — complete on `pkp/architecture-revamp-stage-5-transition-classifier`; route declaration and route target transitions are named in the pure `src/runtime/run/run-transition.ts` classifier, while trace appends, recovery binding checks, Skill Hook dispatch, and terminal close IO stay in the graph runner. Focused transition characterization/classifier tests, runtime tests, Build runtime wiring, Skill Hook actuation, `npm run check-flow-drift`, and `npm run verify` passed.
- Stage 6 shared runtime evidence — complete on `pkp/architecture-revamp-stage-6-runtime-evidence`; shared runtime git snapshot and touched-file schemas now live in `src/schemas/runtime-evidence.ts`, the existing shared projection validates against them, Fix adapters feed `fix.baseline-snapshot@v1` and `fix.change-set@v1` without renaming those reports, and SafeApply/recovery contracts can cite the shared touched-file evidence ref. Focused runtime-evidence, Fix change-set, Fix runtime, guidance, trace, recovery, schema-barrel tests, `npm run check-flow-drift`, and `npm run verify` passed.
- Stage 7 CLI app shell — complete on `pkp/architecture-revamp-stage-7-cli-app-shell`; the root CLI now owns top-level dispatch and version output, while run/resume parsing, fixture loading, config discovery, history recall preparation, runtime calls, recovery continuation, and post-run artifact orchestration live in `src/cli/run.ts`. Focused CLI tests, architecture-boundary tests, `npm run check-flow-drift`, and `npm run verify` passed.
- Stage 7 schema family barrels — complete on `pkp/architecture-revamp-stage-7-schema-barrels`; `run-index`, `flow-index`, `host-index`, `policy-index`, and `evidence-index` now provide reader-focused schema entrypoints while the root barrel still exports every schema file. Schema barrel tests now pin family membership, source-layer isolation, and deliberate overlap. Focused schema-barrel tests, `npm run check`, and `npm run verify` passed.
- Stage 8 convergence cleanup — complete on `pkp/architecture-revamp-stage-8-convergence-cleanup`; references probes kept the flow-kind table checker and catalog-bound flow adapter because they still have CLI/test callers, removed the exported Fix-to-shared-evidence adapter functions after proving they had no importers, refreshed the current architecture maps, and regenerated host runtime bundles. Focused Fix/evidence/schema/architecture tests, `npm run check-flow-drift`, and `npm run verify` passed.
