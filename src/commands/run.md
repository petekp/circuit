...
2. **Build a shell-safe invocation.** Single-quote the raw task text; double
   ...
   ```bash
   ./bin/circuit run review --goal 'review this branch for safety problems' --target main...HEAD --progress jsonl
   ```
...
- `src/cli/run.ts` `resolveCompiledFlowRoute` (explicit-flow requirement; routing is model-only)
- `tests/runner/cli-router.test.ts` (explicit flow routing and the no-flow rejection)
