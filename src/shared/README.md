...
| Runtime evidence and work contracts | `runtime-touched-files.ts`, `work-contract-projection.ts`, `relay-runtime-types.ts` | Stay shared as cross-layer projection helpers. Persisted shapes live in `src/schemas/runtime-evidence.ts`, and recovery route policy lives in `src/policy/recovery-route-policy.ts`. |
