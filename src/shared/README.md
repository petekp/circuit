...
| Fanout and report utilities | `fanout-aggregate-report.ts`, `fanout-branch-template.ts`, `json-extraction.ts`, `zod-to-response-schema.ts` | Stay shared while they remain generic utilities. |
| Runtime evidence and work contracts | `runtime-touched-files.ts`, `work-contract-projection.ts`, `relay-runtime-types.ts`, `git-state.ts`, `git-state-command.ts` | Stay shared as cross-layer projection helpers. Persisted shapes live in `src/schemas/runtime-evidence.ts`, and recovery route policy lives in `src/policy/recovery-route-policy.ts`. `git-state.ts` is a spawn-only subprocess script (never imported); `git-state-command.ts` builds the command that runs it and parses its output for Fix's change-set chain and Build's touch-area gate. |
