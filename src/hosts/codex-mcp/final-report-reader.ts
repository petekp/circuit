...
async function trustedWorkspacePath(workspaceIdentity: {
  ...
  try {
    workspace = await realpath(workspaceIdentity.canonical_path);
  } catch {
    throw new FinalReportReaderError(
  ...
  if (
    ...
    String(workspaceInfo.dev) !== workspaceIdentity.device ||
    String(workspaceInfo.ino) !== workspaceIdentity.inode
  ) {
    throw new FinalReportReaderError(
      'workspace_changed',
      'The trusted Codex workspace changed before Circuit read the report.',
...
// Verifies and reads one digest-bound file inside the run folder: safe path,
// no links, size and hash exactly as recorded at close, identity re-checked
// around the read so the returned bytes are the bytes the locator bound.
async function readVerifiedRunFile(
  workspace: string,
  runId: string,
  locator: { readonly path: string; readonly sha256: string; readonly byte_length: number },
): Promise<Buffer> {
...
export class McpFinalReportReader implements LifecycleReportReader {
  async read(input: Parameters<LifecycleReportReader['read']>[0]) {
    ...
    // The receipt rides along when its bound bytes still verify. A missing or
    // changed receipt is omitted, never fatal: the structured report is the
    // completion's source of truth, the Markdown is its human rendering.
    let operatorSummaryMarkdown: string | undefined;
    if (locator.operator_summary !== undefined) {
      try {
        ...
        operatorSummaryMarkdown = undefined;
      }
    }
    return {
      ...
      ...(operatorSummaryMarkdown === undefined
        ? {}
        : { operator_summary_markdown: operatorSummaryMarkdown }),
    };
  }
}
