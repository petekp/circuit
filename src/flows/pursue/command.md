...
<!--
  Source: flow-owned. Generated host copies live under
  plugins/claude/commands/ and plugins/codex/commands/.
  /circuit:pursue is owned by the pursue flow package, so its source
  of truth lives in src/flows/pursue/.
-->
...
2. **Build a shell-safe invocation.** Single-quote the raw goal text; double
   ...
   ```bash
   ./bin/circuit run pursue --goal 'rework the config loader, then remove the legacy YAML path, then update the docs' --progress jsonl
   ```

   Example for goals containing an apostrophe:

   ```bash
   ./bin/circuit run pursue --goal 'coordinate the can'\''t-ship cleanup goals' --progress jsonl
   ```
