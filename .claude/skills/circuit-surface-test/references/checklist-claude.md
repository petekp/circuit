...
`stopped` trigger: `run goal` against the real connector. **Run it in a
disposable clone, never the real Circuit repo** — `goal`'s child sub-runs
(build/fix/etc.) get trusted-write on cwd, so a concrete objective could edit
your actual checkout (a vague objective often stops at the clarify step before
any child runs, but do not rely on that). `goal` runs a child sub-run and then
evaluates the evidence; a
