...
report. These commands intentionally refresh local host plugin caches before any
checklist row runs. A stale cache is a setup blocker, not a checklist finding.

```bash
git rev-parse HEAD
git status --short
mkdir -p "$REPORT_ROOT/inventory"
npm run plugins:refresh-local > "$REPORT_ROOT/inventory/plugins-refresh-local.out"
git status --short
...
The checklist depends on generated package output and installed host caches
matching the current source tree. Do not fall back to a repo wrapper or an older
installed plugin after a cache check fails.
...
**Evidence streams (where a CLI row's output lands).** With `--progress jsonl`
the per-event progress stream is written to **stderr**, and the final run
envelope (the flat JSON carrying `flow_id`, `outcome`, `selected_flow`,
`operator_summary_status_text`, `run_surface_status_text`, `autonomous_loop`,
`resolved_axes`, etc.) is the trailing pretty-printed object on **stdout**. The
full trace lives in `trace.ndjson` at the **run-folder root**
(`<run-folder>/trace.ndjson`), NOT under `reports/`; per-report artifacts
(`result.json`, `operator-summary.md`, `run-envelope.json`, `build/plan.json`)
sit under `<run-folder>/reports/`. So: grep stderr for `(slice N)` progress
suffixes, and grep `<run-folder>/trace.ndjson` for the 0-based `slice_index`
trace field.
...
- `npm run emit-flows` regenerates the source host package mirrors and bundled
  runtimes before cache sync. `npm run check-plugin-runtime` is the source-tree
  drift check, but a surface test should still run each wrapper's `doctor --json`
  because it checks the packaged runtime, generated command files, hook posture,
  and a temp-repo smoke.
