#!/usr/bin/env bash
# verify-install.sh — Check that all Method plugin prerequisites are met
#
# Usage:
...
if [[ "$FAIL" -gt 0 ]]; then
  printf '\n\033[31mSome checks failed. Fix the issues above before using the Method plugin.\033[0m\n'
  exit 1
else
  printf '\n\033[32mAll checks passed. Method plugin is ready to use.\033[0m\n'
  exit 0
fi
