...
For Build host-surface smoke checks such as `/circuit:run develop: smoke ...`
or `/circuit:build smoke ...`:
...
- First action: run the real bootstrap command directly:
  ...
  "$CIRCUIT_PLUGIN_ROOT/scripts/relay/circuit-engine.sh" bootstrap \
    ...
    --project-root "$PWD"
  ```
- Valid proof is on-disk run state: `.circuit/current-run`,
  `circuit.manifest.yaml`, `events.ndjson`, `state.json`, and
  `artifacts/active-run.md`.
- Never fabricate those files with `Write`, `Edit`, heredocs, or ad hoc shell
  writes.
- Do not run `--help` or search the repo to rediscover the required bootstrap
  flags; use the exact command shape above.
- Stop after validation. Do not continue into planning or broader repo
  exploration for a smoke request.
