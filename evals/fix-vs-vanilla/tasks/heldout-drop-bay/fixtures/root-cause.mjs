...
export function dropBay(plan) {
  const stops = plan.split('>');
  const finalStop = stops[stops.length - 1];
  const pieces = finalStop.split('!');
  if (pieces.length < 2 || pieces[0] === '') return '';
  const rawCode = pieces[pieces.length - 1];
  const parsed = /^([A-Z])0*([0-9]+)$/.exec(rawCode);
  if (parsed === null) return rawCode;
  const [, aisle, stall] = parsed;
  return aisle + stall.padStart(2, '0');
}
